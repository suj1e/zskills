...
**CURRENT 的语义与解析规则**:它是**焦点指针**(无参数命令的默认寻址目标),不是活动战线注册表——活动名单永远从 `runs/*/state.json` 的非终态 status 推导,禁止在 CURRENT 里另建清单双写。文件保持单行 runId,解析规则:

1. **指向 run 已终态**:无参数命令先报告该 run 的收尾摘要,再列出其余非终态战线请用户点名;**不自动切换焦点**(避免惊跳),用户说「切到 <runId>」才改写 CURRENT
2. **换焦点唯一途径 = 显式意图**(「切到 X」/ 启动时的默认写入);并行新战线、收摊老战线都不改写它
3. **缺失/空/指向不存在的 run**:视为无焦点——恰有一个非终态战线则自动认领;零个或多个则列出请用户点名

**全部战线终态后,CURRENT 照常保留、不清空**——书签语义:它仍指向最后一次焦点的 run,是 impl-report 验收报告的天然入口(规则 1 负责递摘要);下一次「跑这批」创建新战线时覆写。主动清空它不会让系统更干净,只会弄丢战后报告入口。
