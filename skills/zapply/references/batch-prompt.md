...
每次启动 batch 先创建本次运行目录 `.zdev/apply/runs/<runId>/`(runId = `<日期>-<HHmm>`,如 `2026-08-27-1540`),三件套同住:`plan.md` / `state.json` / `impl-report.md`;`state.json` 额外写入 **`front`(战线别名)**——从本批 change 的共同前缀/主题提炼(如 `auth`、`hotfix-0808`),用户口头给了名字就用他的。随即将 `.zdev/apply/FOCUS` 写为该 runId。「继续跑」「看进度」只作用于 FOCUS 指向的 run;**历史 run 只读留档,永不复用**。**已在跑时又发起 batch(多战线规则)**:先读 FOCUS 指向 run 的 `state.json`——
...
- 未达终态(analyzing / pending-approval / running / paused) → 停下问用户,三选一:
  ...
  - **(c) 并行新战线**(仅当用户明说"并行",且新批次与**所有**活动战线的 change 核对互不相交):新 run 照开,但 ① `FOCUS` 不动仍指原焦点 run,新战线一律用显式 runId 寻址,「看进度」改报全部非终态战线总览 ② 扫描自动排除所有活动战线占用的 change ③ 全局并发预算:所有战线在跑 craftsman 总数 ≤ 4,预算满则各活动战线并行度 -1、新战线以 1 起步

> 铁律(任何分支生效):**同一个 change 任何时刻至多被一个战线占用**——多 craftsman 同 worktree = tasks.md 互踩、TDD 交叉污染,属于事故而非冲突。

**FOCUS 的语义与解析规则**:它是**焦点指针**(无参数命令的默认寻址目标),不是活动战线注册表——活动名单永远从 `runs/*/state.json` 的非终态 status 推导,禁止在 FOCUS 里另建清单双写。文件保持单行 runId,解析规则:

1. **指向 run 已终态**:无参数命令先报告该 run 的收尾摘要,再列出其余非终态战线请用户点名;**不自动切换焦点**(避免惊跳),用户说「切到 <runId>」才改写 FOCUS
2. **换焦点唯一途径 = 显式意图**(「切到 X」/ 启动时的默认写入);并行新战线、收摊老战线都不改写它
3. **缺失/空/指向不存在的 run**:视为无焦点——恰有一个非终态战线则自动认领;零个或多个则列出请用户点名

**全部战线终态后,FOCUS 照常保留、不清空**——书签语义:它仍指向最后一次焦点的 run,是 impl-report 验收报告的天然入口(规则 1 负责递摘要);下一次「跑这批」创建新战线时覆写。主动清空它不会让系统更干净,只会弄丢战后报告入口。
...
## 验收口径
单项完成 = 三门禁全过 + 代码侧 tasks 全勾(`🔧[人工]` 项不计入,单列人工清单) + 已归档；批次完成 = 本批所有单项到达终态
...
## 待人工执行清单
| # | 动作 | 来源 change | 脚本/命令 | 目标环境 | 验证方式 |
|---|---|---|---|---|---|
<汇总各 change 的 `🔧[人工]` 条目与 design.md「上线与人工动作」；无则写「无需人工动作」>
...
| 「收摊」 | 危险操作:无参时仅当**只有一条**非终态战线 → 复述确认一次后执行;多条 → 必须点名,拒绝猜测 |
| 「切到 <战线>」 | 改写 FOCUS 焦点 |
