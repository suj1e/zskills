...
任何时刻主智能体被要求执行 `git push`（或用户说自己马上要 push），若本会话归档过 change 或存在已完成的 run，**推送前必须扫描**：

1. 扫描范围：本会话归档的 change + CURRENT run 的 impl-report「待人工执行清单」
2. 收集来源中**未勾选**的 🔧[人工] 项（grep tasks.md 的 `- [ ] 🔧[人工]` 行等）
3. 有未执行项 → 逐条列出（动作/目标环境），问用户：

```
⚠️ 推送前确认：以下人工动作是否已执行？
  1. [hotfix-0808] 迁移脚本 migrations/0042_add_index.sql → 生产库
  2. [auth] 控制台回调白名单加 <域名> → 第三方后台
  (a) 都已执行，推送  (b) 先去执行，稍后再推  (c) 照推（自担风险，impl-report 标注）
```

4. 选择 (c) 照推 → 未执行项写入对应 impl-report 的「变更记录」，**不静默**
5. 扫描无未执行项 → 正常推送，不打扰

> 单 change 归档后用户要求 push 同样走此扫描——人工动作与代码先后颠倒，是部署事故的头号来源。
