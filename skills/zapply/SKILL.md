...
**入口分流**:
- **简单改动(主智能体智能识别,或用户说"小修/直接修一下")** → **轻量直改模式**(见下),不开 worktree、不派 craftsman、不开 change
...
**智能识别判据(全满足才允许,存疑即回落完整管线——宁重勿漏)**:
1. 改法显而易见,无需方案设计,不存在"怎么做"的分歧
2. 改动 ≤ 3 个文件,不新增依赖,不改公共接口/数据结构
3. 改完可立即验证(现有测试 / 构建 / 浏览器眼见)
4. 不动 openspec change 体系(纯实现层修正,无需求语义变化)

**五步流程(省仪式不省纪律)**:
1. **改**——主智能体亲自改,不建 worktree、不派子智能体、不写 change 文档
2. **自验**——跑现有测试/构建/眼见确认;不强制补新测试(别把轻量改重了)
3. **规范同步**——改动涉及约定/ skill 文本时,同步 AGENTS.md / test-zs / 相关 SKILL
4. **zpush 安全网**——照常触发,门禁不豁免(人工项/卫生/sanity 照扫)
5. **commit + push**——完成态与重活完全一致 = 写入 + commit + push

判定边界:命中 1-2 条但拿不准 → 按**完整管线**走;用户点名要走完整管线 → 尊重,不省。
...
- 安全网:simplifier 按小步提交逐个意图简化,**每步复跑全部测试**;红了先修后退,不是粗暴全量回滚:
  - 失误可定位 → 有界修复(≤2 轮),修复后复测仍绿则保留该步
  - 修复不动 / 判定简化破坏行为 → 只回滚红的那一步(按单步提交粒度),其余已绿的简化照常保留
  - 根因是测试写死实现细节的脆弱断言 → **不许擅自改测试**,也不许拿回滚掩盖问题,上报主智能体交用户决策
  - 原因查不清 → 回滚本轮全部简化,宁可不简化不带病过门禁
...
**3d-1. 豁免仲裁**:craftsman 回报的每条「不修 + 理由」,按豁免反腐三类(范围外前置 / 跨调用方接口 / 产品级取舍)逐条核对——理由不在三类内即驳回,并入下轮重跑清单;主智能体也拿不准的,升级为用户决策项
...
- 其余情形 merge 完即汇报，形态走 **zshow**：**总结**（合了什么、提交数、归档状态）+ 浅文件树 **diff**（改动落位）——单 change 内联即可
- batch 收官：全景 zshow **落 HTML** 至 `.zdev/show/batch-<runId>.html`（各战线合并摘要 + 汇总文件树 diff），会话内同步给摘要
...
| 「看看批量进度」 | 读 `.zdev/apply/` 下各 run 的 `state.json` / `report.md` 向用户汇报全线与 checkpoint |
...
| **Run 隔离与落盘** | 每 batch 一个 `<runId>/{brief,state.json,report}`:brief 出征冻结 / state 生命体征 / **report 出现即结案**;**零子目录零搬家零记忆态**——活动战线与战后入口全由账本推导,历史删留随意 |
| **多战线规则** | 活动战线未终态时再发 batch 三选一:等(默认)/收摊中止/并行新战线(与所有活动战线不相交+全局 craftsman 预算 ≤4);同 change 任何时刻至多一个战线占用 |
...
- 批量产物按 **run 隔离**:`.zdev/apply/<runId>/` 内含 `brief.md`(决策基线)/`state.json`(运行态)/`report.md`(结案即出现);无任何指针文件与子目录,一切由账本推导;单 change 进度天然在 openspec/changes/ 与 worktree 分支上
...
- `references/run-state.schema.json` — 每次 run 的 `state.json` 状态格式定义(位于 `.zdev/apply/<runId>/`)
