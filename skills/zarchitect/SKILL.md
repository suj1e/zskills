...
> **快车道（先进分流判定，再走步骤）**
> 同时满足 ①根因已明确（architecter 已定性，或问题一目了然）②预计改动面 ≤ 2 个文件 ③无架构取舍、无新依赖 —— 三条齐备才允许上快车道：
...
> **并行提效**：性能劣化、跨模块故障这类重调查输入，在第 1 步就后台预派 `architecter`（拿瓶颈定位与影响面），主智能体同步做需求拆解和澄清提问——不要串行干等它的报告。
...
### 5. 方案设计
在共识基础上输出完整方案。复杂方案(多模块/新架构/跨服务改造)可后台派发 `architecter` 产出实现蓝图骨架(改动清单/影响面/trade-off),主智能体在其上补齐取舍与开放问题;未装则自己写。**必须包含**:
- 背景与目标 / 现有系统分析(如适用)
