...
- 没有 -> **访谈式起草**:问目标/用户/成功标准/范围,按模板成文(brief.md / prd.md),存 `.zdev/`

### 3. 生成评审项(AI 评审官)
通读文档,按框架生成尖锐问题写 `.zdev/review.yaml`(status: reviewing):
...
### 4. 引导对齐
- 在 zdashboard 评审视图中逐项【答复/采纳/驳回】,全部处理完点【通过评审】
- 写操作实时落盘 `.zdev/review.yaml`;关掉 dashboard 进度不丢
